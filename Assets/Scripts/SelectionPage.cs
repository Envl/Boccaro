...
public class SelectionPage : MonoBehaviour {
	...
	void PreManageResource(){
		...
		for(int i=0;i<5;i++){
			// intro x 5
			obj=Resources.Load("SelectionPage/prefab/intro");
			...
			texture2d=Resources.Load("SelectionPage/intro"+(i+1).ToString()) as Texture2D;
			sp=Sprite.Create(texture2d,new Rect(0,0,texture2d.width,texture2d.height),new Vector2(0.5f,1));
			sp.name="intro"+(i+1);
			spr.sprite=sp;
			_intros.Add(instance);
			//docker x 5
			obj=Resources.Load("SelectionPage/prefab/potDocker"+(i+1));
			instance=Instantiate(obj) as GameObject;
			instance.transform.localPosition=new Vector3(i*distBetweenIntro,instance.transform.localPosition.y);
			_potDocks.Add(instance);
			// bigIndicator x 5
			obj=Resources.Load("SelectionPage/prefab/indicator");
			instance=Instantiate(obj)as GameObject;
			instance.transform.localPosition=GenIndicatorPosFromX(_fixedIndicatorXPos[i+4-_frontIndex]);
			spr=instance.GetComponent<SpriteRenderer>();
			texture2d=Resources.Load("SelectionPage/big"+(i+1).ToString()) as Texture2D;
			sp=Sprite.Create(texture2d,new Rect(0,0,texture2d.width,texture2d.height),new Vector2(0.5f,1));
			...
			instance.transform.localPosition=GenIndicatorPosFromX(_fixedIndicatorXPos[i+4-_frontIndex]);
			spr=instance.GetComponent<SpriteRenderer>();
			texture2d=Resources.Load("SelectionPage/small"+(i+1).ToString()) as Texture2D;
			sp=Sprite.Create(texture2d,new Rect(0,0,texture2d.width,texture2d.height),new Vector2(0.5f,1));
			sp.name="indicator"+(i+1);
			spr.sprite=sp;
			//更新透明度
