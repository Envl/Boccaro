...
public class UISpriteAnimation : MonoBehaviour
{
	...
	protected bool mActiveReverse = true;
	protected int mpauseIndex = -1;
	...
	/// <summary>
	/// Returns is the the currentIndex of the animation
	/// </summary>

	public int getIndex { get { return mIndex; } }
	...
	protected virtual void Update ()
	{
		...
		else if (mActiveReverse && mSpriteNames.Count > 1 && Application.isPlaying && mFPS > 0)
		{
			mDelta += Mathf.Min(1f, RealTime.deltaTime);
			float rate = 1f / mFPS;
			...
			while (rate < mDelta)
			{
				...
				if (--mIndex < 0)
				{
					...
					mActiveReverse = mLoop;
				}

				if (mIndex == mpauseIndex)
					mActiveReverse = false;
	...
	//play to numth picture then pause
	public void PlayTo (int num) { mActive = true;mpauseIndex = num;  }
	//返回放有问题，差了两帧
	public void PlayToReverse(int num) { mActiveReverse = true; mpauseIndex = num; }
	public void resetFrame (int frame) { mFPS = frame;  }
	/// <summary>
	/// Pause the animation.
	/// </summary>

	public void Pause () { mActive = false; mActiveReverse = false; }
